...
Take-home for Nortex Industries.
...
understanding, assumptions, what was cut, where it breaks.
...
Email + password for the fixed Nortex roster — there is no self-service account
...
| `SEED_DEFAULT_PASSWORD` | `Nortex@2026`                                    | password given to every seeded employee                                                                  |
...
configuration, sets `Nortex@2026` on every account, and — only when the claims
database is otherwise empty — builds **one draft trip** for Chaitanya Reddy by
running the supplied 15 emails and 2 receipts through the real ingestion
...
| -------------------------------------- | ------------------ | ----------------------------------------------------------- |
| `chaitanya.reddy@nortexindustries.com` | Employee           | the demo trip; personal reimbursement + policy analytics    |
| `suresh.iyer@nortexindustries.com`     | Reporting Manager  | Approvals queue; approver analytics                         |
| `meera.krishnan@nortexindustries.com`  | Head of Department | Approvals queue (higher-value claims)                       |
| `arvind.rao@nortexindustries.com`      | Head of Division   | Approvals queue                                             |
| `ravi.menon@nortexindustries.com`      | Finance            | Finance queue; verification / payment / exception analytics |
| `nandita.shah@nortexindustries.com`    | MD                 | Approvals + org-wide oversight                              |
| `admin@nortexindustries.com`           | Admin              | org dashboard + a read-only claim register                  |
...
- **The pack is the specification.** City tiers, the approval matrix, the
  entitlement caps, and the non-reimbursable list all come from
  `expense_policy.md`; the roster and reporting lines from `employee_master.csv`.
...
Built to run the one supplied trip end to end and be honest about the edges.
Deliberately out of scope for the timebox:

- **General OCR.** Extraction is deterministic for the two supplied receipt
  images (`ingestion/known-receipts.ts`); an unrecognised image becomes a
  `needs_info` line the claimant completes. The pipeline still runs.
...
Assumptions and known rough edges are in
