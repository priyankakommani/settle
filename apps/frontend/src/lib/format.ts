...
/** Strips anything but letters, spaces, and `.'-` as the user types — mirrors the backend's city name validator. */
export function sanitizeCityInput(value: string): string {
  return value.replace(/[^A-Za-z\s.'-]/g, '');
}

/** true if the ISO timestamp falls on the local calendar's today. */
