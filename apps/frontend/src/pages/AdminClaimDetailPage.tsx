    ...
    <AsyncSection loading={false} error={q.error} data={q.data}>
      {(detail) => (
        <>
          ...
          <Card><CardHeader title={`Source documents (${detail.documents.length})`} /><CardBody><DocumentsList tripId={detail.trip.id} documents={detail.documents} claimLines={detail.claimLines} grid /></CardBody></Card>
        </>
      )}
    </AsyncSection>
  );
